    ...
    const groupedSavedCampaigns = useMemo(() => {
        ...
        sortedGroupKeys.forEach(groupId => {
            ...
            const groupTotalAmount = group.items.reduce((sum, item) => {
                ...
                const reviewType = row.reviewType ?? item.reviewType;
                const quantity = Number(row.quantity ?? item.quantity);
                const cDate = item.date?.seconds ? new Date(item.date.seconds * 1000) : new Date();
                const reviewFee = getBasePrice(deliveryType, reviewType) + (cDate.getDay() === 0 ? 600 : 0);
                const productPriceWithAgencyFee = Number(item.productPrice) * 1.1;
                const subtotal = (reviewFee + productPriceWithAgencyFee) * quantity;
                const finalAmount = item.isVatApplied ? subtotal * 1.1 : subtotal;
                return sum + finalAmount;
            }, 0);
    
            const groupName = group.items[0].createdAt
                ? format(new Date(group.items[0].createdAt.seconds * 1000), 'yy.MM.dd')
                : '개별 등록';
            ...
            group.items.forEach((item, index) => {
                flatList.push({
                    ...
                    groupTotalAmount: Math.round(groupTotalAmount),
                    groupName: groupName,
                });
            });
        });
