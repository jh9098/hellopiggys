...
export default function SellerTrafficPage() {
  ...
  return (
    <>
      ...
      {confirmRequest && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setConfirmRequest(null)}><div className="bg-white p-6 rounded shadow" onClick={(e) => e.stopPropagation()}><p className="mb-4">예약확정 하시겠습니까?</p><div className="flex justify-center space-x-4"><button className="px-4 py-2 bg-blue-600 text-white rounded" onClick={handleConfirmReservation}>예</button><button className="px-4 py-2 bg-gray-300 rounded" onClick={() => setConfirmRequest(null)}>아니오</button></div></div></div>)}
      {showDepositPopup && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={handleClosePopupAndReset}><div className="bg-white p-8 rounded-lg shadow-xl text-center" onClick={(e) => e.stopPropagation()}><h3 className="text-xl font-bold mb-4">입금 계좌 안내</h3><p className="text-lg">아래 계좌로 <strong className="text-green-600">{remainingPayment.toLocaleString()}원</strong>을 입금해주세요.</p><div className="my-6 p-4 bg-gray-100 rounded-md"><p className="font-semibold text-lg mb-1">채종문 (아이언마운틴컴퍼니)</p><p className="font-semibold text-2xl text-blue-700">국민은행 834702-04-290385</p></div><p className="text-sm text-gray-600 mb-6">입금 확인 후 '나의 트래픽 예약 내역'에서 상태가 변경됩니다.</p><button className="w-full px-4 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700" onClick={handleClosePopupAndReset}>확인</button></div></div>)}
    </>
  );
}