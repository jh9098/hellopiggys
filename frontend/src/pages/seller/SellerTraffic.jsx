// src/pages/seller/SellerTraffic.jsx (Vite 환경에 맞게 수정된 최종본)

import { useState, useMemo, useEffect } from 'react';
import { useNavigate } from 'react-router-dom'; // [수정]
import { db, auth, onAuthStateChanged, collection, serverTimestamp, query, where, onSnapshot, writeBatch, doc, increment, updateDoc } from '../../firebaseConfig'; // [수정]
...
export default function SellerTrafficPage() {
  const [user, setUser] = useState(null); // [수정]
  const [isLoading, setIsLoading] = useState(true); // [수정] 로딩 상태 추가
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        setIsLoading(false);
        // Clean up Firestore listeners when user logs out or component unmounts
        return () => {
            unsubscribeRequests();
            unsubscribeSeller();
        };
        ...
        setUser(null);
        setIsLoading(false);
        navigate('/seller-login'); // 사용자가 없으면 로그인 페이지로 이동
      }
    });
